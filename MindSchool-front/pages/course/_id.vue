  ...
  <div id="aCoursesList" class="bg-fa of">
    ...
    <section class="container">
      <section class="path-wrap txtOf hLh30">
        ...
        \
        <a
          :href="'/course?subjectParentId='+courseWebVo.subjectLevelOneId"
          class="c-333 fsize14"
        >{{courseWebVo.subjectLevelOne}}</a>
        \
        <a
          :href="'/course?subjectParentId='+courseWebVo.subjectLevelOneId+'&subjectId='+courseWebVo.subjectLevelTwoId"
          class="c-333 fsize14"
        >{{courseWebVo.subjectLevelTwo}}</a>
      </section>
      <div>
            ...
            <section class="c-attr-mt of">
              <span v-if="isCollect" class="ml10 vam sc-end">
                ...
                <a
                  ...
                  class="c-fff vam"
                  title="取消收藏"
                  @click="removeCollect(courseWebVo.id)"
                >已收藏</a>
              </span>
...
export default {
  ...
  methods: {
    ...
    //生成订单
    createOrders() {
      orderApi.createOrders(this.$route.params.id).then(response => {
        //获取返回订单号
        //生成订单之后，跳转订单显示页面
        this.$router.push({
    ...
    initComment() {
      commentApi
        .getPageList(this.page, this.limit, this.courseWebVo.id)
        .then(response => {
    ...
    //跳转页面
    gotoPage(page) {
      commentApi.getPageList(page, this.limit, this.courseWebVo.courseId).then(response => {
        this.data = response.data.data;
      });
    }
  }
